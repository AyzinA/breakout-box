#!/system/bin/sh
# Universal secure upgrader for Magisk modules.
# The same file can be used unchanged across modules.
# Supports both multi-module and legacy single-module update.json manifests.
...
# Universal upgrader defaults. Module defaults may override these.
AUTO_UPDATE=false
...
LOG_FILE="$STATE_DIR/upgrader.log"
STATUS_FILE="$STATE_DIR/upgrader.status"
LOCK_DIR="$STATE_DIR/upgrade.lock"
PID_FILE="$STATE_DIR/upgrader.pid"

# Persistent config always wins over packaged defaults.
[ -r "$CONFIG_FILE" ] && . "$CONFIG_FILE"

# Re-evaluate if persistent config intentionally overrides MODULE_STATE_DIR.
...
mkdir -p "$STATE_DIR" "$UPGRADE_DIR" 2>/dev/null
chmod 700 "$STATE_DIR" "$UPGRADE_DIR" 2>/dev/null
...
rotate_logs() {
    ...
    case "$UPGRADER_MAX_LOG_SIZE" in ''|*[!0-9]*) UPGRADER_MAX_LOG_SIZE=1048576 ;; esac
    case "$UPGRADER_MAX_LOG_FILES" in ''|*[!0-9]*) UPGRADER_MAX_LOG_FILES=3 ;; esac

    [ "$SIZE" -gt "$UPGRADER_MAX_LOG_SIZE" ] || return 0

    [ -f "${LOG_FILE}.${UPGRADER_MAX_LOG_FILES}" ] && rm -f "${LOG_FILE}.${UPGRADER_MAX_LOG_FILES}"

    I=$((UPGRADER_MAX_LOG_FILES - 1))
    while [ "$I" -ge 1 ]; do
...
log_msg() {
    ...
    command -v log >/dev/null 2>&1 && log -t "${MODULE_ID}-upgrader" "[$LEVEL] $MESSAGE" 2>/dev/null
}
...
# Extract one object from a multi-module manifest where the object key is the
# module ID, e.g. "breakout-box": { ... }. This avoids requiring jq on Android.
extract_module_manifest() {
...
select_manifest_for_module() {
    INPUT="$1"; OUTPUT="$2"

    # Preferred v2 format: { "modules": { "<module-id>": { ... } } }
    if extract_module_manifest "$INPUT" "$OUTPUT" "$MODULE_ID" && [ -s "$OUTPUT" ]; then
        log_msg INFO "Selected manifest entry for module=$MODULE_ID"
        return 0
    fi

    # Backward-compatible v1 format: one module object at the JSON root.
    LEGACY_ID="$(json_string moduleId "$INPUT")"
    if [ "$LEGACY_ID" = "$MODULE_ID" ]; then
        cp -f "$INPUT" "$OUTPUT" 2>/dev/null || return 1
        log_msg INFO "Using legacy single-module manifest for module=$MODULE_ID"
        return 0
    fi

    rm -f "$OUTPUT"
    log_msg ERROR "No update manifest entry found for module=$MODULE_ID"
    return 1
}
...
validate_zip() {
    ...
    TEMP_PROP="$UPGRADE_DIR/module.prop.new"
...
check_once() (
    if ! mkdir "$LOCK_DIR" 2>/dev/null; then
        log_msg INFO "Upgrade check skipped because another operation is active"
        return 0
    fi
    ...
    if bool_true "$UPDATE_INSTALL_ONLY_ON_WIFI" && ! wifi_connected; then
        log_msg INFO "Upgrade skipped because Wi-Fi is not connected"
        cleanup_lock; trap - EXIT INT TERM HUP; return 0
    fi

    MANIFEST="$UPGRADE_DIR/update.json"
    SELECTED_MANIFEST="$UPGRADE_DIR/module-update.json"
    ZIP="$UPGRADE_DIR/module-upgrade.zip"
    rm -f "$MANIFEST" "$MANIFEST.part" "$SELECTED_MANIFEST" "$ZIP" "$ZIP.part"

    log_msg INFO "Checking for upgrades; module=$MODULE_ID installed=$CURRENT_VERSION_CODE"
    ...
    select_manifest_for_module "$MANIFEST" "$SELECTED_MANIFEST" || { cleanup_lock; trap - EXIT INT TERM HUP; return 1; }

    NEW_CODE="$(json_uint versionCode "$SELECTED_MANIFEST")"
    NEW_VERSION="$(json_string version "$SELECTED_MANIFEST")"
    ZIP_URL="$(json_string zipUrl "$SELECTED_MANIFEST")"
    EXPECTED_SHA="$(json_string sha256 "$SELECTED_MANIFEST" | tr 'A-F' 'a-f')"
    ...
    is_uint "$NEW_CODE" || { log_msg ERROR "Manifest versionCode is invalid for module=$MODULE_ID"; cleanup_lock; trap - EXIT INT TERM HUP; return 1; }

    if [ "$NEW_CODE" -le "$CURRENT_VERSION_CODE" ]; then
        log_msg INFO "No upgrade available; module=$MODULE_ID installed=$CURRENT_VERSION_CODE remote=$NEW_CODE"
        printf '%s\n' "up-to-date module=$MODULE_ID installed=$CURRENT_VERSION_CODE remote=$NEW_CODE" > "$STATUS_FILE"
        cleanup_lock; trap - EXIT INT TERM HUP; return 0
    fi

    secure_url "$ZIP_URL" || { log_msg ERROR "Manifest zipUrl is missing or insecure"; cleanup_lock; trap - EXIT INT TERM HUP; return 1; }
    log_msg INFO "Downloading upgrade module=$MODULE_ID version=${NEW_VERSION:-unknown} versionCode=$NEW_CODE"
    download_file "$ZIP_URL" "$ZIP" || { log_msg ERROR "Failed to download upgrade ZIP"; cleanup_lock; trap - EXIT INT TERM HUP; return 1; }
    ...
    if bool_true "$UPDATE_REQUIRE_SHA256"; then
        ...
        [ "$ACTUAL_SHA" = "$EXPECTED_SHA" ] || { log_msg ERROR "SHA-256 mismatch; upgrade rejected"; rm -f "$ZIP"; cleanup_lock; trap - EXIT INT TERM HUP; return 1; }
    ...
    command -v magisk >/dev/null 2>&1 || { log_msg ERROR "Magisk command unavailable; cannot install upgrade"; cleanup_lock; trap - EXIT INT TERM HUP; return 1; }
    log_msg INFO "Installing upgrade module=$MODULE_ID version=${NEW_VERSION:-unknown} versionCode=$NEW_CODE"

    if magisk --install-module "$ZIP" >> "$LOG_FILE" 2>&1; then
        rotate_logs
        log_msg INFO "Upgrade installed successfully; reboot required"
        printf '%s\n' "installed module=$MODULE_ID version=${NEW_VERSION:-unknown} versionCode=$NEW_CODE reboot_required=true" > "$STATUS_FILE"
        bool_true "$UPDATE_KEEP_DOWNLOADED_ZIP" || rm -f "$ZIP"
        cleanup_lock; trap - EXIT INT TERM HUP; return 0
    fi

    rotate_logs
    log_msg ERROR "Magisk rejected the module upgrade"
    printf '%s\n' "install-failed module=$MODULE_ID versionCode=$NEW_CODE" > "$STATUS_FILE"
    cleanup_lock; trap - EXIT INT TERM HUP; return 1
)

run_loop() {
    bool_true "$AUTO_UPDATE" || { log_msg INFO "AUTO_UPDATE=false; automatic upgrader disabled"; return 0; }
    ...
    log_msg INFO "Upgrade loop started: module=$MODULE_ID initial_delay=${INITIAL_DELAY}s interval=${INTERVAL}s"
    sleep "$INITIAL_DELAY"
    while true; do
        check_once
        sleep "$INTERVAL"
...
show_status() {
    ...
    echo "UPGRADER_MAX_LOG_SIZE: $UPGRADER_MAX_LOG_SIZE"
    echo "UPGRADER_MAX_LOG_FILES: $UPGRADER_MAX_LOG_FILES"
    echo "Manifest: $UPDATE_JSON_URL"
    echo "Log: $LOG_FILE"
    [ -r "$STATUS_FILE" ] && { echo "Last status:"; cat "$STATUS_FILE"; } || echo "Last status: none"
    if [ -r "$PID_FILE" ]; then
        PID="$(cat "$PID_FILE" 2>/dev/null)"
        if is_uint "$PID" && kill -0 "$PID" 2>/dev/null; then echo "Upgrader loop: running (PID $PID)"; else echo "Upgrader loop: stale PID"; fi
    else
        echo "Upgrader loop: stopped"
    fi
}
